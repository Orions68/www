...
$title = "Modificando los Datos del Alumno";
...
if (isset($_POST["id"])) // Si llegan datos por post.
{
    ...
    {
        $sql = "SELECT phone, email FROM alumno WHERE id=$id;"; // Preparo la consulta a la ID del cliente.
        $stmt = $conn->prepare($sql);
        $stmt->execute();
    ...
    {
        echo "<script>toast(1, 'Ya Registrado:', 'El E-mail $email o el Teléfono $phone, ya Están Registrados en Este Sitio y Alguno es de otro Alumno. No Puedes Usar Datos de Otro Alumno, si haz Cambiado el Teléfono Hace Poco, Comunícate con la Academia.');</script>"; // Muestro el error, el E-mail ya está registrado.
    }
}
