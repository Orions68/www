...
if (isset($_POST["email"]))
{
    ...
    $ok = false;
    $sql = "SELECT email FROM alumno;"; // Preparo una consulta de todos los E-mail de espectadores de la base de datos.
    ...
    {
        $sql = "UPDATE alumno SET pass='$pass' WHERE email='$email'"; // Hago un update de la contraseña de ese E-mail.
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        echo "<script>toast(0, 'Todo ha Ido Bien', 'Se ha cambiado tu Contraseña a: $hash, Selecciónala y Cópiala, Después Vuelve a Iniciar Sesión con los Nuevos Datos. Te Recomendamos que Cambies la Contraseña, Gracias.');</script>";
    }
}
?>
